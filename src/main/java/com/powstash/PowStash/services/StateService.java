...
public class StateService implements StateServiceInterface {
    ...
    public List<MountainDto> getMountainsByState(int id) {
        ...
        return list.stream().map(mountain -> new MountainDto(mountain.getId(), mountain.getName(),mountain.getDescription(), mountain.getAddress(),mountain.getLongitude(),mountain.getLatitude(),mountain.getLocalKnowledge(), mountain.getState().getId(),mountain.getPass().getId())).toList();
    }
